...
void InitUObject()
{
	...
	/* 注册模块加载的
	*  这里的重点是ProcessNewlyLoadedUObjects的注册，
	*  这个函数的构造生成类型系统一个非常重要的函数。
	*  我们知道，UE的组织形式是Module，一个Module编译后可以生成一个dll。
	*  dll是可以动态加载的，因此如果在引擎初始化结束后，继续动态加载一个模块（即dll)，
	*  根据C++机制，会触发dll里面的static变量初始化。因此元数据信息就又收集到了一些。
	*  我们就需要继续利用这些元数据信息来为这个新dll里定义的类构造类型的UClass*对象。
	*  总而言之，注册这个事件的目的是让新的模块在加载完毕后，让我们为dll里的native类构造类型对象。
	*  一个非常重要要知道的是，这里隐含的另一个意思是ProcessNewlyLoadedUObjects有可能会被调用多次。
	*/ 
...
// 
// UObjectBaseInit初始化结束后，就已经可以开始NewObject了，
// 标志着整个UObject系统的成功创建！
// GObjTransientPkg是个全局变量，所有没有Outer的对象都会放在这个包里。
// 我们在NewObject的时候，如果不提供Outer，则会返回这个临时包，
// 符合了UObject对象必须在UPackage里的一贯基本原则。
//
void StaticUObjectInit()
{
	//继续转发
