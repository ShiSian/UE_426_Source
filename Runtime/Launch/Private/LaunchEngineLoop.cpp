...
int32 FEngineLoop::PreInitPreStartupScreen(const TCHAR* CmdLine)
{
...

	{
		...
		//加载一些PreInit的模块，比如Engine，Renderer
		LoadPreInitModules();
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	// Start the application
	{
		...
		//AppInit():程序初始化
		//通知了程序初始化事件，让别的代码有机会继续进行一些初始化工作。
		if (!AppInit())
		{
			return 1;
		}
...
int32 FEngineLoop::PreInitPostStartupScreen(const TCHAR* CmdLine)
{
...

	{
		...
		//自己写的LoadingPhase为PreDefault的模块在这个时候加载
	...
	if (GUObjectArray.IsOpenForDisregardForGC())
	{
		...
		//对象池启用（开启GC），最开始是无视GC的，也就是关闭的
		// 
		//因为之前一直都是在初始化载入阶段，这个阶段构造的类型UClass*对象和CDO对象及其属于的UPackage对象，都是属于引擎底层的必要对象，
		//它们是只有在游戏退出的时候才会销毁，因此它们就不归GC管---GC一开始也是关着的：OpenForDisregardForGC = true。
		//在类型系统都构建完了之后，就可以打开GC了，因为后续就有可能NewObject来生成对象了。
		GUObjectArray.CloseDisregardForGC();
	}
	//注册完成事件通知，完成Package加载
...
* 【涉及UObject启动的最开始的地方】
* UE是建立在UObject对象系统上的，
* 所以引擎里别的模块想要启动加载起来，就得先把CoreUObject模块初始化完成。
* 因此引擎循环的预初始化部分就得开始加载CoreUObject了。
*/
...
* 
* 该函数主要做一些其他程序入口的初始化工作
* 最后一步用一个多播委托通知了程序初始化事件，让别的代码有机会继续进行一些初始化工作。
*/
